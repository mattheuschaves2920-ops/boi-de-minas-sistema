...
def dashboard():
    ...
    return render_template("dashboard.html", user=current_user(), faturamento=faturamento, faturamento_padaria=faturamento_padaria,
                           refeicoes=refeicoes, custo=custo, lucro=lucro, alertas=alertas, por_periodo=por_periodo,
                           producao_custo=producao_custo, desperdicio_valor=desperdicio_valor, vendidos_diarios=vendidos_diarios)
...
def itens():
    ...
    if request.method == "POST":
        ...
        if existing:
            ...
            db.session.add(Item(area=area, code=code, name=name, category=request.form.get("category", "").strip(),
...
def controle_diario():
    ...
    data_ref = datetime.strptime(filtro, "%Y-%m-%d").date() if filtro else date.today()
    ...
    return render_template("controle_diario.html", user=current_user(), lista=lista, daily_groups=DAILY_GROUPS, data_ref=data_ref, totais=totais)
...
def relatorios():
    ...
    return render_template("relatorios.html", user=current_user(), total_items=total_items, total_alertas=total_alertas,
                           ...
                           ticket_medio=ticket_medio, total_producao=total_producao, total_diario=total_diario, por_periodo=por_periodo)
