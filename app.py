...
class Movement(db.Model):
    ...
    mov_type = db.Column(db.String(20), nullable=False)  # Entrada, Saida, Perda
    area = db.Column(db.String(40), nullable=False)
...
def dashboard():
    ...
    custo = sum(m.value for m in moves_today if m.mov_type in ["Saida", "Perda"])
    lucro = faturamento - custo
    alertas = [i for i in items if i.stock <= i.min_stock]

    por_tipo = {}
    for s in sales_today:
        por_tipo[s.meal_type] = por_tipo.get(s.meal_type, 0) + s.quantity
    ...
    latest_moves = Movement.query.order_by(Movement.id.desc()).limit(10).all()
    return render_template("dashboard.html", user=current_user(), faturamento=faturamento, refeicoes=refeicoes,
                           custo=custo, lucro=lucro, alertas=alertas, por_tipo=por_tipo,
                           por_periodo=por_periodo, latest_moves=latest_moves)
...
def itens():
    ...
    return render_template("itens.html", user=current_user(), itens=itens, areas=AREAS)
...
def relatorios():
    ...
    total_perdas = db.session.query(db.func.sum(Movement.value)).filter(Movement.mov_type == "Perda").scalar() or 0
    ...
    return render_template("relatorios.html", user=current_user(), total_items=total_items,
                           total_alertas=total_alertas, total_perdas=total_perdas,
                           total_vendas=total_vendas, total_refeicoes=total_refeicoes,
                           ticket_medio=ticket_medio, por_periodo=por_periodo)
...
def api_resumo():
    ...
    custo = sum(m.value for m in moves_today if m.mov_type in ["Saida","Perda"])
    lucro = faturamento - custo
    return jsonify({"faturamento": faturamento, "refeicoes": refeicoes, "custo": custo, "lucro": lucro})
